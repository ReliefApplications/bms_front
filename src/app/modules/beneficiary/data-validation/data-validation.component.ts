...
export class DataValidationComponent implements OnInit {
    ...
    */
	...
	step2Duplicates(data: any, type: string, idDuplicate: string, newHousehold: any, idCache?: number, state?: boolean) {
        ...
        for (var id in this.correctedData[key].data) {
          if (this.correctedData[key].data.hasOwnProperty[id]) {
            // search if the duplicate id is already in correctedData
  					...
  					if (id === idDuplicate) {
  						...
  						if (type == 'old') {
  							if (state) {
  								this.correctedData[key].data[id].state = state;
  							} else {
  								this.correctedData[key].data[id].state = !this.correctedData[key].data[id].state;
  							}
  							// when state is true, add an object to_delete containing name of new object
  							if (this.correctedData[key].data[id].state) {
  								this.correctedData[key].data[id].to_delete = {};
  								this.correctedData[key].data[id].to_delete.given_name = data.new.households.beneficiaries[0].given_name;
  								this.correctedData[key].data[id].to_delete.family_name = data.new.households.beneficiaries[0].family_name;
  							}
  							// when state is false, delete object to_delete
  							else if (!this.correctedData[key].data[id].state) {
  								delete this.correctedData[key].data[id].to_delete;
  							}
  						} else if (type == 'new') {
  							// when new already exist, delete it
  							// if new exist, it means checkbox is already checked and we want to uncheck it
  							if (this.correctedData[key].data[id].new) {
  								delete this.correctedData[key].data[id].new;
  								this.correctedData[key].data[id].to_delete = {};
  								this.correctedData[key].data[id].to_delete.given_name = data.new.households.beneficiaries[0].given_name;
  								this.correctedData[key].data[id].to_delete.family_name = data.new.households.beneficiaries[0].family_name;
  							}
  							// when new doesn't exist, create it
  							// if new doesn't exist, it means it isn't checked and we want to check it
  							else {
  								this.correctedData[key].data[id].new = data.new.households;
  								delete this.correctedData[key].data[id].to_delete;
  							}
  						}
  					}
      ...
      let correctDuplicate = new FormatDuplicatesData;
        ...
        for (var key in this.correctedData) {
    			if (this.correctedData.hasOwnProperty(key)) {
            if (key === idCache) {
              this.correctedData[idCache]= verification;
              alreadyExist = true;
            }
          }
        }
			...
			if (!alreadyExist) {
				...
				correctDuplicate.data[idDuplicate] = verification;

				this.correctedData[idCache] = correctDuplicate;
			}
		}
    ...
    /**
     * Used to send data to the backend with correction after every step
     * Data can only be sent if all data is verified
     */
	sendCorrectedData() {

...
export class ChartComponent implements OnInit, ChartInterface, OnChanges, AfterViewChecked {
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if ((changes.filters.currentValue !== changes.filters.previousValue && !ChartRegistration.comparaisons.get(this.uniqId)) ||
            ...
            (changes.periodFrequency && changes.periodFrequency.currentValue !== changes.periodFrequency
              && changes.periodFrequency.previousValue)) {
    ...
    /**
  	 ...
  	 */
    getData() {
        const promise = this._chartDataLoaderService.load(this.indicatorConfig.idIndicator, this.body);
        if (promise) {
    ...
    /**
  	 * function called when there is an interaction with the date button
  	 */
    ...
    /**
  	 * search the filter associated to the charts
  	 */
    chartAssociation() {
